...
## Active Work
- [2026-06-18] **Cross-platform port (Windows + macOS)** — repo now runs on a Mac (`~/Documents/AI World/agents`, monorepo, in sync). Full punch list: `chief-of-staff/docs/cross-platform-port-plan.md`; decision: `wiki/decisions/2026-06-17-cross-platform-agent-runtime.md`. Locked: **Mac primary, Windows secondary, cloud owns cadence, everything→Mac, no hardcoded absolute paths.** DONE: Atlas wiki/agent paths → repo-relative; Atlas python hooks OS-neutral (`python3||python`, live next session). TODO: Polaris owns Bucket 1 remainder (python in dev-agent, workspace.local.json, repo-relative scripts, path-leak scanner) — work order in `wiki/log.md` 2026-06-18. **Bucket 5:** move inter-agent Slack (#atlas-cos, #polaris-tl) from DaFudge → WDAI workspace (only WDAI is wired on Mac; DaFudge pings fail here). advisor() gate waived for this internal port.
