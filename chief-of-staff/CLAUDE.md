...
- FTS5 index for `/recall` rebuilds on session start via the SessionStart hook
...
- **`wiki/`** changes → propose commits, ask Dina before push (shared with Polaris — coordinate).
...
- On session start, run `/heartbeat` once. No loop, no scheduler.
...
| `Atlas\Promote` | Daily 11:00 PM | Extract learnings from daily logs → memory (curates Session Log: collapses runs of near-identical entries) |
...
| `Atlas\Decay` | Weekly Sun 4:15 AM | Move daily logs >180d into `memory/archive-YYYY-MM.md` |
