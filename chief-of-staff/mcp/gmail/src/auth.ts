/**
 * OAuth helper — run this to authenticate each Google account for Gmail.
...
const oauth2 = new google.auth.OAuth2(client_id, client_secret, "http://localhost:3334/callback");
...
console.log(`\nAuthenticating Gmail account: "${label}"`);
console.log(`\nOpen this URL in your browser:\n\n${authUrl}\n`);
console.log("Waiting for callback on http://localhost:3334/callback ...\n");
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    res.end(`<h2>Gmail "${label}" authenticated!</h2><p>You can close this tab.</p>`);

    console.log(`Token saved to ${tokenPath}`);
    console.log(`Gmail account "${label}" is ready.`);
