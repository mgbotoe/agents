...
- **WDAI Compound Engineering (active):** PR #689 MERGED (2026-06-23). CE plugin adopted. Open: (1) **CE scope issue** — plugin installed at project scope only; doesn't load in Polaris-rooted sessions. Fix: user-scope install via `/plugin install compound-engineering@compound-engineering-plugin` (awaiting Dina's choice); (2) mark `Check for destructive migrations` as required CI check — **mgbotoe HAS admin on wdai-foundation-platform (verified 2026-06-24), can do this directly**; (3) fast-follow: AGENTS.md `docs/solutions/` pointer once ce-compound runs; (4) cross-tool CE optional per-machine.
...
- **Pending Dina:** (1) `ctx-upgrade` — context-mode v1.0.75 broken (EBUSY); requires closing Claude Code first. (2) `slack-watcher/test-singleton.mjs` — commit or delete. (3) self-improve.yml needs repo setting "Allow GitHub Actions to create and approve pull requests" enabled. (4) `/advisor` → council vs fork: confirm fork approach. (5) CE user-scope install: run `/plugin install compound-engineering@compound-engineering-plugin` (or flip registry record project→user with instances closed).
...
- **CE plugin scope rule:** A plugin must be *installed* at `user` scope (not merely enabled) to load in arbitrary sessions. Project-scoped install only loads when that project is the session root. Hand-editing `installed_plugins.json` is racy with a live instance.
...
- **mgbotoe has FULL ADMIN on `WomenDefiningAI/wdai-foundation-platform`** (verified 2026-06-24 via `gh api`: `permissions.admin=true`). Can bypass branch protection, merge over `REVIEW_REQUIRED` (`gh pr merge --admin`), edit rulesets, and make CI checks required. Corrects the prior "neither Dina nor mgbotoe has admin" belief. (Separate repo: still read-only on `WomenDefiningAI/mailchimp-cc`.) Admin capability ≠ standing authorization — PR/merge actions still need Dina's explicit per-action word.
