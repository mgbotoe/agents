...
- [2026-04-19] **Agent ecosystem roadmap** created at `wiki/projects/agent-ecosystem.md`. Living doc parallel to `wdai-tech-debt.md` but for agent infrastructure. Seeded with P0 (polling mirror — now shipped), P1 (cross-post migration at N=3, Atlas's 5 UX items), P2 (watcher hardening, distill short-circuit, log rotation, runtime state centralization), P3 (shared agent-core, sub-agent SDK). Both agents can add. **Scope rule:** agent infra → this roadmap; external projects → per-project doc; agent-specific internals → that agent's memory.
- [2026-04-19] **Polaris symmetric inbox polling shipped** (commit `9cc35fc`). Heartbeat skill reads `#atlas-cos` since `.claude/runtime/atlas-last-seen.ts` watermark (self-bootstraps on missing-file), updates watermark to newest ts after processing. SessionStart hook also echoes inbox reminder on every spawn. Mirrors Atlas's `polaris-last-seen.ts` pattern. Closes the real-time gap in Polaris→Atlas direction where the watcher self-filters. Tested end-to-end manually — filter logic + watermark advancement verified. Real-world validation happens at next scheduled Distill spawn.
