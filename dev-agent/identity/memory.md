...
- **Decay (Polaris + Atlas):** now cloud cron `.github/workflows/decay.yml` (matrix both agents, Sun 09:00 UTC), replacing the dead Windows Task Scheduler jobs. First real archival run ~Oct 2026 once 180-day cutoff bites — verify on first non-empty run.
- **Cross-platform (Mac + Windows, sequential use):** Polaris now OS-agnostic. Path derivation via `parents[N]` (no hardcoded `C:\`), `.mcp.json.example` + `config/secrets.env.example` templates, `.gitattributes`, `bin/setup-mac.sh`. `gather-context.ps1` ported → `gather-context.py` (also fixed the 0GB disk bug via `shutil.disk_usage`). Heartbeat now runs via `/loop` (local-state watchdog, no cloud home). Self-improve revived as cloud cron `self-improve.yml` → `bin/self-improve.py` — **PR-gated** (opens a PR, never writes master; security.md/SOUL.md/CLAUDE.md off-limits to auto-edit). All Windows Task Scheduler `.cmd`/`.bat` + `gather-context.ps1` deleted; `bin/scheduled/` gone. Scheduling = cloud cron + hooks + `/loop`, all OS-agnostic.
...
- **Pending Dina:** (1) `ctx-upgrade` — context-mode v1.0.75 broken (EBUSY); requires closing Claude Code first. (2) `slack-watcher/test-singleton.mjs` — commit or delete decision. (3) Enable repo setting "Allow GitHub Actions to create and approve pull requests" (Settings→Actions→General) or self-improve.yml's PR step fails. (4) On the Mac: clone into `<root>/agents/dev-agent`, run `bin/setup-mac.sh`, fill `config/secrets.env`.
...
## Standing Rules
- **Do NOT route WDAI decisions/actions to Helen by default.** Dina is CAIO and the decision-owner for WDAI. Helen being "founder/owner" in `wdai-foundation-platform/AGENTS.md` is a fact, not a routing instruction — don't reflexively assign approvals, confirmations, or "send this to Helen" steps to her. If something genuinely needs repo-admin access Dina lacks, ask Dina who holds admin / whether she does — don't name Helen by default. (Corrected by Dina twice, 2026-06-13.)
...
- [2026-06-13] Adopted Compound Engineering plugin team-wide on WDAI foundation repo (PR #689, docs-only); kept all WDAI skills, advisor-validated; caught that destructive-migration guard already exists in ci.yml (avoided duplicate). Synced agents repo (9 commits). Standing Rule added: don't route WDAI decisions to Helen.
